...
export const ListeningHero = ({ vocab, onBack }: { vocab: Vocabulary[]; onBack: () => void }) => {
  ...
  if (vocab.length < 4) {
    return (
      <div className="text-center p-20 bg-white dark:bg-stone-900 rounded-[3rem] border border-stone-100 dark:border-stone-800 shadow-xl">
        ...
        <p className="text-stone-500 dark:text-stone-400 mb-8 font-serif italic">Add at least 4 words to your vocabulary to play Listening Hero.</p>
    ...
    <div className="max-w-2xl mx-auto space-y-8">
      <div className="flex justify-between items-center">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-2xl font-editorial italic text-stone-900 dark:text-stone-100">Listening Hero</h2>
            <p className="text-stone-500 dark:text-stone-400 font-serif italic text-xs">Listen and pick the correct word.</p>
          </div>
        </div>
      ...
      <div className="bg-white dark:bg-stone-900 p-12 rounded-[3rem] border border-stone-100 dark:border-stone-800 shadow-xl text-center space-y-10">
        <button 
          ...
          className="w-32 h-32 bg-stone-900 dark:bg-stone-100 text-white dark:text-stone-900 rounded-full flex items-center justify-center mx-auto shadow-2xl hover:scale-105 transition-all active:scale-95 group"
        >
          <Volume2 className="w-12 h-12 group-hover:animate-pulse" />
        </button>
        <p className="text-stone-400 dark:text-stone-500 font-serif italic">Meaning: {currentQuestion?.word.meaning}</p>
